  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        if (ref) {
          const parentTop = ref.parentElement.getBoundingClientRect().top + window.scrollY;
    ...
    <main className='bg-gradient-to-b from-[#fdf1e7] to-white px-5 md:px-15 lg:px-30'>
      ...
      <div className="pt-4 md:pt-8 lg:pt-12 pb-4">
        <div className="max-w-8xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-30">
          <div>
            <h5 className="text-sm md:text-base font-semibold text-black pt-4 md:pt-6">Awards</h5>
            <h1 className='text-2xl md:text-4xl lg:text-5xl font-semibold pt-3 md:pt-8 leading-tight'>
              Award-winning <br />digital marketing
            </h1>
            <p className="text-md lg:text-lg font-semibold pt-4 md:pt-8 pb-6 md:pb-8">
          ...
          <div className="flex flex-col gap-10 md:gap-16 lg:gap-20 justify-around">
            {awards.map((award, i) => (
              <div key={i}>
                ...
                <p className="text-sm md:text-base text-gray-500 pt-1">{award.subtitle}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#ebebf4] mb-20 px-2 md:px-4 lg:px-6 pt-6 md:pt-10 lg:pt-16 pb-10 md:pb-11 lg:pb-22 rounded-[55px] max-w-8xl mx-auto text-center">
        ...
        <div className="bg-[#ededf0] rounded-lg px-6 py-4 shadow-md mb-6">
          ...
          <input type="range" min="1" max="100" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full h-2 rounded-lg cursor-pointer appearance-none bg-[#e0e0e0]" style={{ background: `linear-gradient(to right, #f89e1b 0%, #facc15 ${price}%, #e0e0e0 ${price}%, #e0e0e0 100%)`, accentColor: 'black'}}/>
          <div className="text-right text-xs text-gray-500 mt-1">{price}</div>
        </div>
        <div className="text-5xl font-bold text-[#f89e1b] mb-6">
          ₹{Number(price).toFixed(2)} <span className="text-gray-500 text-2xl font-medium">/month</span>
        </div>
