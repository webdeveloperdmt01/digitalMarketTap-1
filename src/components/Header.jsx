...
import targetIcon from "../assets/popup-arrow.webp";

const Header = () => {
  const [marketingSolutionsVisible, setMarketingSolutionsVisible] =
    useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !marketingRef.current.contains(event.target)
      ) {
    ...
    <>
      <header className="bg-[#fff7ee] flex items-center justify-between px-5 md:px-10 lg:px-15 xl:px-30">
        ...
        <nav className="font-semibold font-syne hidden lg:flex items-center space-x-6 text-sm relative">
          ...
          <div
            ref={marketingRef}
            onMouseEnter={() => setMarketingSolutionsVisible(true)}
            className="relative"
          >
            <NavLink
              to="/marketing-solution"
              onMouseEnter={() => setMarketingSolutionsVisible(true)}
              className={({ isActive }) =>
                `font-semibold text-sm border-b-2 flex items-center ${
                  isActive || marketingSolutionsVisible
                    ? "border-[#F89E1B]"
                    : "border-transparent"
                }`
              }
            ...
            {marketingSolutionsVisible && (
              <div className="absolute top-full justify-around left-[160px] -translate-x-1/2 mt-8 bg-white shadow-lg rounded-3xl z-50 px-5 py-6 flex space-x-10 w-[1260px]">
                <div className="flex flex-col space-y-1">
                  <HoverService
                    icon={<FaCode color="#000" />}
                    text="Website Development"
                    to="/website-development"
                  />
                  <HoverService
                    icon={<FaSearch color="#E0E0E0" />}
                  ...
                  <HoverService
                    icon={<FaCommentDots color="#8e81fa" />}
                ...
                <div className="flex flex-col space-y-1">
                  <HoverService
                    icon={<FaImage color="#097B11" />}
                    text="Graphic Designing"
                    to="/graphic-design"
                  />
                  <HoverService
                    icon={<FaHashtag color="purple" />}
                ...
                <div className="bg-[#fff2df] rounded-3xl px-5 pt-10 pb-6 flex flex-col items-center w-[280px] relative">
                  <img
                    src={targetIcon}
                    alt="target"
                    className="w-20 h-25 absolute -top-5 right-0 -translate-x-1/2"
                  />
                  <div className="w-full flex items-center justify-between mt-auto">
                    <Link
                      to="/marketing-solution"
                      className="font-syne text-sm font-semibold hover:text-white"
                    >
                      Explore all <br /> solutions
                    </Link>
                    <Link to="/marketing-solution" ><FaArrowRight className="bg-black text-white rounded-[12px] p-2 text-3xl hover:bg-white hover:text-[#8e81fa]" /></Link>
                  </div>
                </div>
              </div>
            )}
...
const HoverService = ({ icon, text, to = "#" }) => {
  ...
  return (
    ...
    >
      <div className="bg-[#f7f7f7] text-[#F89E1B] p-3 rounded-full text-lg">
        {icon}
      </div>
      <span className="font-medium text-black hover:text-[#8e81fa]">
        {text}
      </span>
    </NavLink>
  );
...
const DropdownItem = ({ icon, text, to = "#" }) => (
  ...
  >
    <div className="bg-[#fff5ea] text-[#F89E1B] p-2 rounded-full text-lg">
      {icon}
    </div>
